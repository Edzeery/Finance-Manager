...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; border-bottom: 3px solid var(--accent); padding-bottom: 20px; }
        .header-left { display: flex; align-items: center; gap: 12px; }
        .logo-icon { width: 42px; height: 42px; background: linear-gradient(135deg, var(--accent), #4f46e5); border-radius: 10px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 18px; font-weight: 700; }
        .app-name { font-size: 20px; font-weight: 700; color: #1e1b4b; }
        .header-right { text-align: right; }
        .invoice-number { font-size: 22px; font-weight: 700; color: var(--accent); margin-bottom: 4px; }
        ...
        .badge-draft { background: var(--border); color: #374151; }
        ...
        .info-card { flex: 1; background: var(--bg); border-radius: 8px; padding: 14px 16px; border: 1px solid var(--border); }
        ...
        th { background: #f1f5f9; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); }
        th, td { padding: 10px 14px; text-align: left; }
        th:last-child, td:last-child { text-align: right; }
        td { border-bottom: 1px solid var(--border); font-size: 12px; }
        .row-subtotal td { border-bottom: none; }
        .row-total td { border-top: 3px solid var(--accent); font-weight: 700; font-size: 16px; background: var(--bg); padding: 14px; }
...
<body>
    ...
    <div class="header">
        <div class="header-left">
            ...
            <div>
                ...
                <div style="font-size:11px;color:var(--text-muted)">{{ __('settings.invoice') }}</div>
            </div>
        </div>
    ...
    <div style="display:flex;gap:16px;margin-bottom:24px">
        <div style="flex:1;background:var(--bg);border-radius:8px;padding:14px 16px;border:1px solid var(--border)">
        ...
        <div style="flex:1;background:var(--bg);border-radius:8px;padding:14px 16px;border:1px solid var(--border)">
            ...
            @if($invoice->user?->email)
                <div style="font-size:11px;color:var(--text-muted)">{{ $invoice->user->email }}</div>
            @endif
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr class="row-total">
                ...
                <td style="color:var(--accent)">{{ $displayPrice($invoice->total, $invoice->currency) }}</td>
            </tr>

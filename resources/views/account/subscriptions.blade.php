...
<x-app-layout>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
                    ...
                    @if($subscription && $subscription->plan)
                        ...
                        <div style="background:linear-gradient(135deg, var(--accent), color-mix(in srgb, var(--accent) 70%, #000));margin:-1.25rem -1.25rem 1.25rem -1.25rem;padding:1.5rem 1.75rem;position:relative">
                            ...
                            <div class="d-flex align-items-center justify-content-between gap-3" style="position:relative;z-index:1">
                                <div>
                                    <div style="font-size:13px;color:rgba(255,255,255,0.7);margin-bottom:4px">{{ __('settings.current_plan') }}</div>
                                    <h3 style="font-weight:700;color:#fff;margin-bottom:4px">{{ $plan->name }}</h3>
                                    @if($plan->isFree())
                                        <span style="font-size:15px;color:rgba(255,255,255,0.9)">{{ __('settings.free_plan') }}</span>
                                    @else
                                        <span style="font-size:20px;font-weight:700;color:#fff">{{ $displayPrice($plan->monthly_price) }}</span>
                                        <span style="font-size:13px;color:rgba(255,255,255,0.7)">/{{ __('general.month') }}</span>
                                        @if($plan->yearly_price > 0)
                                            <span style="font-size:13px;color:rgba(255,255,255,0.6);margin-inline-start:8px">{{ $displayPrice($plan->yearly_price) }}/{{ __('general.year') }}</span>
                                        @endif
                                    @endif
                                    @if($plan->description)
                                        <p style="font-size:13px;color:rgba(255,255,255,0.7);margin-top:4px;margin-bottom:0">{{ $plan->description }}</p>
                                    @endif
                                </div>
                        ...
                        <div class="row g-4 mb-4">
                            <div class="col-md-6">
                                ...
                                <div>
                                    <div class="d-flex justify-content-between text-muted-sm" style="margin-bottom:4px">
                                        <span><i class="bi bi-people" style="margin-inline-end:4px"></i>{{ __('settings.users_usage') }}</span>
                            ...
                            <div class="col-md-6">
                                ...
                                <div>
                                    <div class="d-flex justify-content-between text-muted-sm" style="margin-bottom:4px">
                                        <span><i class="bi bi-arrow-left-right" style="margin-inline-end:4px"></i>{{ __('settings.transactions_usage') }}</span>
                                        <span>{{ $txCount }} / {{ $txLimit }} {{ __('general.transactions') }}</span>
                                    </div>
                                    <div class="progress" style="height:6px;border-radius:3px;background:var(--border)">
                                        <div class="progress-bar" role="progressbar"
                                             style="width:{{ $txPercent }}%;border-radius:3px;background:{{ $txPercent > 80 ? 'var(--danger)' : ($txPercent > 60 ? 'var(--warning)' : 'var(--accent)') }}"
                                             aria-valuenow="{{ $txPercent }}" aria-valuemin="0" aria-valuemax="100">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row g-0" style="border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-bottom:1rem">
                            ...
                            <div class="col-4" style="border-inline-end:1px solid var(--border);background:var(--bg-subtle)">
                                <div style="padding:12px 16px;text-align:center">
                                    ...
                                    <div class="text-muted-sm" style="font-size:11px;margin-bottom:2px">{{ __('settings.payment_method') }}</div>
                                    <div style="font-weight:600;font-size:13px">{{ $_gatewayName }}</div>
                                    <div style="font-size:11px;color:var(--text-muted);margin-top:1px">{{ $_methodTypeLabel }}</div>
                                </div>
                            </div>
                            <div class="col-4" style="border-inline-end:1px solid var(--border);background:var(--bg-subtle)">
                                <div style="padding:12px 16px;text-align:center">
                                    <i class="bi bi-calendar-event" style="font-size:16px;color:var(--text-muted);margin-bottom:4px;display:block"></i>
                                    <div class="text-muted-sm" style="font-size:11px;margin-bottom:2px">{{ __('settings.billing_period') }}</div>
                                    <div style="font-weight:600;font-size:13px">{{ $subscription->billing_period === 'yearly' ? __('general.yearly') : __('general.monthly') }}</div>
                                </div>
                            </div>
                            <div class="col-4" style="border-inline-end:1px solid var(--border);background:var(--bg-subtle)">
                                <div style="padding:12px 16px;text-align:center">
                                    <i class="bi bi-hourglass-split" style="font-size:16px;color:var(--text-muted);margin-bottom:4px;display:block"></i>
                                    <div class="text-muted-sm" style="font-size:11px;margin-bottom:2px">{{ __('settings.days_remaining') }}</div>
            ...
            <div class="settings-section">
                <div class="settings-card">
                    <h5 class="section-title mb-1 d-flex align-items-center gap-2"><i class="bi bi-receipt text-accent"></i><span>{{ __('settings.payment_history') }}</span></h5>
            ...
            <div class="settings-section">
                <div class="settings-card">
                    <h5 class="section-title mb-1 d-flex align-items-center gap-2"><i class="bi bi-clock-history text-accent"></i><span>{{ __('settings.subscription_history') }}</span></h5>
                        ...
                        <table class="table table-custom mb-0">
                            ...
                            <tbody>
                                @foreach($allSubscriptions as $sub)
                                    ...
                                    <tr>
                                        <td>
                                            <span style="font-weight:500">{{ $sub->plan?->name ?? __('general.unknown') }}</span>
                                            @if($sub->workspace)
                                                <div style="font-size:12px;color:var(--text-muted)">{{ $sub->workspace->name }}</div>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td style="font-size:13px;color:var(--text-muted)">{{ $sub->ends_at?->format('Y/m/d') ?? '—' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="settings-section">
                <div class="settings-card">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <h5 class="section-title mb-1 d-flex align-items-center gap-2"><i class="bi bi-file-text text-accent"></i><span>{{ __('settings.invoices') }}</span></h5>
        ...
        <div class="col-lg-4" id="available-plans">
            <div class="settings-section" style="position:sticky;top:24px">
                <div class="settings-card">
                    <h5 class="section-title mb-1 d-flex align-items-center gap-2"><i class="bi bi-grid-3x3-gap text-accent"></i><span>{{ __('settings.available_plans') }}</span></h5>
                            ...
                            <div class="plan-card mb-3 {{ $isCurrent ? 'plan-current' : '' }}" style="border:1px solid {{ $isCurrent ? 'var(--accent)' : 'var(--border)' }};border-radius:12px;padding:16px;transition:all 0.2s" x-data="{ showAll: false }">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div>
                                        ...
                                        @if($plan->is_free)
                                            <span style="font-size:13px;color:var(--text-muted)">{{ __('settings.free') }}</span>
                                        @else
                                            <span style="font-size:20px;font-weight:700">{{ $displayPrice($plan->monthly_price) }}
                                                <span style="font-size:12px;font-weight:400;color:var(--text-muted)">/{{ __('general.month') }}</span>
                                            </span>
                                            @if($plan->yearly_price > 0)
                                ...
                                <div style="font-size:12px;color:var(--text-muted);margin-bottom:12px">
                                    <i class="bi bi-people"></i> {{ __('admin.users_count') }}: {{ $plan->max_users }}
                                </div>

                                @if(!$isCurrent)
                                    <form action="{{ route('account.subscriptions.change-plan') }}" method="POST" class="plan-form" onsubmit="return handlePlanSubmit(this)">
                                        @csrf
                                        ...
                                        @if(!$plan->is_free)
                                            ...
                                            <div class="mb-2">
                                             ...
                                             <div class="fee-breakdown-{{ $plan->id }}" style="display:none"></div>
                                         @endif
    ...
    function updateFeeBreakdown(form) {
        ...
        fetch('{{ route('account.subscriptions.fee-breakdown') }}?' + params.toString())
            ...
            .then(function(data) {
                ...
                html += '<div class="price-row original"><span>{{ __('onboarding.plan_price') }}</span><span>' + fmt(data.original) + '</span></div>';
                if (parseFloat(data.discount_usd) > 0) {
                    html += '<div class="price-row discount"><span>{{ __('onboarding.coupon_discount') }}</span><span>-' + fmt(data.discount) + '</span></div>';
                }
                if (parseFloat(data.gateway_fee_usd) > 0) {
                    html += '<div class="price-row fee"><span>{{ __('onboarding.gateway_fee') }}</span><span>+' + fmt(data.gateway_fee) + '</span></div>';
                }
                if (parseFloat(data.tax_added_usd) > 0) {
                    html += '<div class="price-row fee"><span>{{ __('onboarding.tax_added') }}</span><span>+' + fmt(data.tax_added) + '</span></div>';
                }
                if (parseFloat(data.tax_disclosed_usd) > 0) {
                    html += '<div class="price-row"><span>{{ __('onboarding.tax_disclosed') }}</span><span>' + fmt(data.tax_disclosed) + '</span></div>';
                }
                html += '<div class="price-divider"></div>';
                html += '<div class="price-row total' + (parseFloat(data.total_usd) <= 0 ? ' free' : '') + '"><span>{{ __('onboarding.total') }}</span><span>';
    ...
    function validateCoupon(input) {
        ...
        fetch('{{ route('coupon.validate', ['code' => '__CODE__', 'amount' => '__AMOUNT__']) }}'
            ...
            .then(data => {
                if (data.valid) {
                    const label = data.type === 'percentage' ? data.value + '%' : '{{ config('finance.currency_symbol') }}' + data.discount;

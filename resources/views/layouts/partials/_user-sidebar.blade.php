<aside class="sidebar" id="mainSidebar">
    ...
    <div class="sidebar-workspace" x-data="{ wsOpen: false }">
        <button class="workspace-switcher" type="button" @click="wsOpen = !wsOpen" @click.away="wsOpen = false">
            ...
            <span class="workspace-info" x-show="!collapsed" x-cloak>
                ...
                @if($planName)
                    <span class="workspace-plan-badge" style="font-size:10px;color:var(--accent);font-weight:600;display:block;margin-top:1px">{{ $planName }}</span>
                @endif
            </span>
                ...
                @foreach($allWorkspaces as $ws)
                    <form method="POST" action="{{ route('workspace.switch', $ws) }}">
                        ...
                        <button type="submit" class="workspace-dropdown-item">
                            <span class="ws-dot {{ $currentWs && $currentWs->id === $ws->id ? 'active-ws' : '' }}">{{ substr($ws->name, 0, 1) }}</span>
                            <span>{{ $ws->name }}</span>
    ...
    <nav class="sidebar-body">
        ...
        <div class="sidebar-nav-section" x-data="{ open: true }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('general.overview') }}">
            ...
            <div x-show="open">
                <a href="{{ route('dashboard') }}" class="sidebar-nav-item {{ request()->routeIs('dashboard') ? 'active' : '' }}" wire:navigate data-label="{{ __('general.dashboard') }}">
        ...
        <div class="sidebar-nav-section" x-data="{ open: {{ request()->routeIs('transactions.*') || request()->routeIs('income.*') || request()->routeIs('expense.*') ? 'true' : 'false' }} }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('general.finances') }}">
        ...
        <div class="sidebar-nav-section" x-data="{ open: {{ request()->routeIs('budget.*') || request()->routeIs('goal.*') ? 'true' : 'false' }} }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('general.planning') }}">
        ...
        <div class="sidebar-nav-section" x-data="{ open: {{ request()->routeIs('asset.*') || request()->routeIs('debt.*') ? 'true' : 'false' }} }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('general.assets_liabilities') }}">
        ...
        <div class="sidebar-nav-section" x-data="{ open: {{ request()->routeIs('zakat.*') || request()->routeIs('report.*') || request()->routeIs('activity.logs') ? 'true' : 'false' }} }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('general.more') }}">
        ...
        {{-- Account --}}
        <div class="sidebar-nav-section" x-data="{ open: {{ request()->routeIs('account.*') || request()->routeIs('two-factor.*') ? 'true' : 'false' }} }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('general.account') }}">
                <i class="bi bi-person-circle section-toggle-icon"></i>
                <span x-show="!collapsed" x-cloak>{{ __('general.account') }}</span>
                <i class="bi bi-chevron-down section-chevron" :class="open ? '' : 'collapsed'" x-show="!collapsed" x-cloak></i>
            </button>
            <div x-show="open" style="display:none">
                <a href="{{ route('account.profile') }}" class="sidebar-nav-item {{ request()->routeIs('account.profile') ? 'active' : '' }}" wire:navigate data-label="{{ __('general.profile') }}">
                    <i class="bi bi-person-fill"></i>
                    <span x-show="!collapsed" x-cloak>{{ __('general.profile') }}</span>
                </a>
                <a href="{{ route('account.subscriptions') }}" class="sidebar-nav-item {{ request()->routeIs('account.subscriptions') ? 'active' : '' }}" wire:navigate data-label="{{ __('settings.subscriptions') }}">
                    <i class="bi bi-credit-card-fill"></i>
                    <span x-show="!collapsed" x-cloak>{{ __('settings.subscriptions') }}</span>
                </a>
                <a href="{{ route('account.invoices.index') }}" class="sidebar-nav-item {{ request()->routeIs('account.invoices.*') ? 'active' : '' }}" wire:navigate data-label="{{ __('settings.invoices') }}">
                    <i class="bi bi-receipt"></i>
                    <span x-show="!collapsed" x-cloak>{{ __('settings.invoices') }}</span>
                </a>
                <a href="{{ route('account.settings') }}" class="sidebar-nav-item {{ request()->routeIs('account.settings') ? 'active' : '' }}" wire:navigate data-label="{{ __('general.preferences') }}">
                    <i class="bi bi-sliders2"></i>
                    <span x-show="!collapsed" x-cloak>{{ __('general.preferences') }}</span>
                </a>
                ...
                <a href="{{ route('two-factor.setup') }}" class="sidebar-nav-item" wire:navigate data-label="{{ __('general.security') }}">
                    ...
                    <span x-show="!collapsed" x-cloak>{{ __('general.security') }}</span>
                </a>
            </div>
        </div>
        ...
        <div class="sidebar-nav-section" x-data="{ open: {{ request()->routeIs('settings.*') || request()->routeIs('workspace.*') ? 'true' : 'false' }} }">
            <button class="sidebar-section-toggle" @click="open = !open" type="button" data-label="{{ __('workspace.team') }}">
                <i class="bi bi-people section-toggle-icon"></i>
                <span x-show="!collapsed" x-cloak>{{ __('workspace.team') }}</span>
                <i class="bi bi-chevron-down section-chevron" :class="open ? '' : 'collapsed'" x-show="!collapsed" x-cloak></i>
            </button>
            <div x-show="open" style="display:none">

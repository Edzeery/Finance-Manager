...
<body class="error-layout"
    style="--error-gradient:linear-gradient(135deg,var(--warning),var(--danger));--error-icon-bg:var(--warning-light);--error-icon-color:var(--warning)">
    <x-page-loader />
    <div class="error-page">
        ...
        <div class="error-actions">
            ...
            @auth
                 <a href="{{ route('dashboard') }}" class="btn-error btn-error-primary"><i class="bi bi-house-door"></i>
                {{ __('general.dashboard') }}</a>
            @endauth


            <a href="javascript:location.reload()" class="btn-error btn-error-secondary"><i
                    class="bi bi-arrow-counterclockwise"></i> {{ __('Try Again') }}</a>
        </div>
    </div>
</body>

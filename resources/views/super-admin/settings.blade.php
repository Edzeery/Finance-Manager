    ...
    <div x-data="settingsTabs()" x-init="initTabs()">
            ...
            <div class="col-12 col-md-3">
                <div class="settings-tabs-sidebar">
                    <nav class="nav flex-column nav-pills settings-nav">
                        <button @click="switchTab('general')" :class="{ active: tab === 'general' }"
                            class="settings-nav-link">
                            <i class="bi bi-sliders ms-2"></i>{{ __('super-admin.tab_general') }}
                        </button>
                        <button @click="switchTab('system')" :class="{ active: tab === 'system' }"
                            class="settings-nav-link">
                            <i class="bi bi-gear-wide-connected ms-2"></i>{{ __('super-admin.tab_environment') }}
                        </button>
                        <button @click="switchTab('security')" :class="{ active: tab === 'security' }"
                            class="settings-nav-link">
                            <i class="bi bi-shield-check ms-2"></i>{{ __('super-admin.tab_security') }}
                        </button>
                        <button @click="switchTab('payments')" :class="{ active: tab === 'payments' }"
                            class="settings-nav-link">
                            <i class="bi bi-credit-card-2-front ms-2"></i>{{ __('super-admin.tab_payments') }}
                        </button>
                        <button @click="switchTab('exchangeRates')" :class="{ active: tab === 'exchangeRates' }"
                            class="settings-nav-link">
                            <i class="bi bi-currency-exchange ms-2"></i>{{ __('super-admin.tab_exchange_rates') }}
                        </button>
                        <button @click="switchTab('rateLimits')" :class="{ active: tab === 'rateLimits' }"
                            class="settings-nav-link">
                            <i class="bi bi-speedometer2 ms-2"></i>{{ __('super-admin.tab_rate_limits') }}
                        </button>
                        <button @click="switchTab('currencies')" :class="{ active: tab === 'currencies' }"
                            class="settings-nav-link">
                            <i class="bi bi-currency-exchange ms-2"></i>{{ __('super-admin.tab_currencies') }}
                        </button>
                        <button @click="switchTab('zakatPrices')" :class="{ active: tab === 'zakatPrices' }"
                            class="settings-nav-link">
                    ...
                    <div class="row g-4 mb-4">
                                    ...
                                    <form method="POST" action="{{ route('super.admin.settings.update') }}">
                                        ...
                                        <div class="form-floating-group">
                                            <input type="text" name="app_name" id="app_name" class="form-control"
                                                placeholder="{{ __('general.app_name') }}"
                                        ...
                                        <div class="form-floating-group">
                                            <select name="default_locale" id="default_locale" class="form-control">
                                                <option value="en"
                                                    {{ ($settings['default_locale'] ?? config('app.locale')) === 'en' ? 'selected' : '' }}>
                                                    English</option>
                                                <option value="ar"
                                                    {{ ($settings['default_locale'] ?? config('app.locale')) === 'ar' ? 'selected' : '' }}>
                                                    العربية</option>
                                                <option value="fr"
                                                    {{ ($settings['default_locale'] ?? config('app.locale')) === 'fr' ? 'selected' : '' }}>
                                                    Français</option>
                                            </select>
                                            ...
                                            <select name="base_currency" id="base_currency" class="form-control">
                                                ...
                                                @foreach (\App\Helpers\CurrencyHelper::availableCurrencies() ?: [['code' => 'USD', 'name' => 'US Dollar'], ['code' => 'DZD', 'name' => 'Algerian Dinar'], ['code' => 'EUR', 'name' => 'Euro']] as $cur)
                                                    <option value="{{ $cur['code'] }}"
                                                        {{ $baseCurrencySetting === $cur['code'] ? 'selected' : '' }}>
                                                        {{ $cur['code'] }} - {{ $cur['name'] }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <x-toggle-switch class="mb-3" name="registration_enabled" value="1"
                                            :checked="($settings['registration_enabled'] ?? '1') === '1'" label="{{ __('super-admin.registration_enabled') }}"
                                            hint="{{ __('super-admin.registration_hint') }}" />
                                    <x-button variant="accent" icon="bi bi-check2" submit>{{ __('general.save') }}</x-button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="section-card">
                                ...
                                @php
                                    $healthBadgeBg =
                                        $systemInfo['system_health'] === 'healthy'
                                            ? 'var(--success-light)'
                                            : 'var(--danger-light)';
                                    $healthBadgeColor =
                                        $systemInfo['system_health'] === 'healthy' ? 'var(--success)' : 'var(--danger)';
                                @endphp
        ...
        ['label' => __('super-admin.app_url'), 'value' => $systemInfo['app_url'], 'type' => 'code'],
                                            ...
                                            <div class="d-flex align-items-center px-4 py-2"
                                                ...
                                                <span
                                                    style="width:150px;font-size:12.5px;color:var(--text-muted);flex-shrink:0">{{ $row['label'] }}</span>
                                                <span style="font-size:13px;color:var(--text)">
                                                    @if (($row['type'] ?? '') === 'badge')
                                                        <span class="badge"
                                                            style="font-size:10px;background:{{ $row['bg'] }};color:{{ $row['color'] }};padding:3px 10px;border-radius:6px;font-weight:600">{{ $row['value'] }}</span>
                                                    @else
                                                        <code>{{ $row['value'] }}</code>
                                                    @endif
                                                </span>
                                        ...
                                        <div class="form-floating-group">
                                            <select name="app_env" class="form-control">
                                                <option value="production"
                                                    {{ ($systemSettings['app_env'] ?? 'local') === 'production' ? 'selected' : '' }}>
                                                    {{ __('super-admin.production') }}</option>
                                                <option value="local"
                                                    {{ ($systemSettings['app_env'] ?? 'local') === 'local' ? 'selected' : '' }}>
                                                    Local</option>
                                                <option value="testing"
                                                    {{ ($systemSettings['app_env'] ?? 'local') === 'testing' ? 'selected' : '' }}>
                                                    Testing</option>
                                                <option value="staging"
                                                    {{ ($systemSettings['app_env'] ?? 'local') === 'staging' ? 'selected' : '' }}>
                                                    Staging</option>
                                            </select>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <input type="text" class="form-control"
                                                value="{{ ($systemSettings['app_debug'] ?? 'false') === 'true' ? __('general.enabled') : __('general.disabled') }}"
                                                readonly disabled>
                                            <label>{{ __('super-admin.app_debug') }}</label>
                                            <div class="form-hint"><i class="bi bi-lock"></i>
                                                {{ __('super-admin.app_debug_readonly') }}</div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <select name="session_driver" class="form-control">
                                                @foreach (['database', 'redis', 'file', 'array', 'cookie'] as $driver)
                                                    <option value="{{ $driver }}"
                                                        {{ ($systemSettings['session_driver'] ?? config('session.driver', 'database')) === $driver ? 'selected' : '' }}>
                                                        {{ ucfirst($driver) }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <input type="url" name="app_url" class="form-control"
                                                placeholder=" "
                                                value="{{ $systemSettings['app_url'] ?? config('app.url') }}">
                                        ...
                                        <div class="form-floating-group">
                                            <select name="log_level" class="form-control">
                                                @foreach (['debug', 'info', 'notice', 'warning', 'error', 'critical', 'alert', 'emergency'] as $level)
                                                    <option value="{{ $level }}"
                                                        {{ ($systemSettings['log_level'] ?? 'warning') === $level ? 'selected' : '' }}>
                                            ...
                                            <select name="log_channel" class="form-control">
                                                @foreach (['single', 'daily', 'slack', 'syslog', 'errorlog', 'stack'] as $channel)
                                                    <option value="{{ $channel }}"
                                                        {{ ($systemSettings['log_channel'] ?? 'daily') === $channel ? 'selected' : '' }}>
                                                        {{ ucfirst($channel) }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <select name="session_encrypt" class="form-control">
                                                <option value="false"
                                                    {{ ($systemSettings['session_encrypt'] ?? 'false') === 'false' ? 'selected' : '' }}>
                                                    {{ __('general.disabled') }}</option>
                                                <option value="true"
                                                    {{ ($systemSettings['session_encrypt'] ?? 'false') === 'true' ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <select name="session_secure_cookie" class="form-control">
                                                <option value="false"
                                                    {{ ($systemSettings['session_secure_cookie'] ?? 'false') === 'false' ? 'selected' : '' }}>
                                                    {{ __('general.disabled') }}</option>
                                                <option value="true"
                                                    {{ ($systemSettings['session_secure_cookie'] ?? 'false') === 'true' ? 'selected' : '' }}>
                                                    {{ __('general.enabled') }}</option>
                                            </select>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <select name="session_same_site" class="form-control">
                                                <option value="lax"
                                                    {{ ($systemSettings['session_same_site'] ?? 'lax') === 'lax' ? 'selected' : '' }}>
                                                    Lax</option>
                                                <option value="strict"
                                                    {{ ($systemSettings['session_same_site'] ?? 'lax') === 'strict' ? 'selected' : '' }}>
                                                    Strict</option>
                                                <option value="none"
                                                    {{ ($systemSettings['session_same_site'] ?? 'lax') === 'none' ? 'selected' : '' }}>
                                                    None</option>
                                            </select>
                        ...
                        <div class="section-card-body">
                            @if ($twoFactorEnabled)
                                <div class="d-flex align-items-center gap-2 mb-3"
                                    style="font-size:13px;background:var(--success-light);color:var(--success);border-radius:var(--radius-sm);padding:10px 14px">
                                ...
                                <form method="POST" action="{{ route('super.admin.settings.2fa.disable') }}">
                                    @csrf @method('PUT')
                                    <x-button variant="outline-danger" icon="bi bi-shield-slash" onclick="confirmDisable2fa(this)">{{ __('general.disable') }}</x-button>
                                </form>
                            @else
                                <div class="d-flex align-items-center gap-2 mb-3"
                                    style="font-size:13px;background:var(--warning-light);color:var(--warning);border-radius:var(--radius-sm);padding:10px 14px">
                                    <x-status-icon domain="general" status="warning" set="bi" />
                                    {{ __('auth.2fa_not_enabled') }}
                                </div>
                                <x-button href="{{ route('two-factor.setup') }}" variant="accent" icon="bi bi-shield-plus">{{ __('general.setup_2fa') }}</x-button>
                            @endif
                        </div>
                    </div>
                </div>
                    ...
                    <div class="section-card mb-4">
                        ...
                        <div class="section-card-body">
                            ...
                            <div class="p-4 text-center">
                                ...
                                <p class="text-muted mt-2 mb-3">{{ __('super-admin.credentials_managed_in_methods') }}
                                </p>
                                <x-button href="{{ route('super.admin.payment-methods.index') }}" variant="accent" icon="bi bi-gearms-1">{{ __('super-admin.manage_payment_methods') }}</x-button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="tab === 'exchangeRates'" x-cloak x-transition:enter.duration.200ms>
                    @php
                        $exchangeRates = json_decode(\App\Models\Setting::get('exchange_rates', '{}'), true) + [
                            'USD' => 1,
                            'DZD' => 250,
                            'EUR' => 0.877,
                            'GBP' => 0.75,
                            ...
                            <form method="POST" action="{{ route('super.admin.settings.exchange-rates.update') }}">
                                ...
                                <div class="row g-3">
                                    ...
                                    @foreach ($rateCurrencies as $cur)
                                        <div class="col-md-4">
                                            <div class="form-floating-group">
                                                <input type="number" name="rates[{{ $cur }}]"
                                                    class="form-control" placeholder=" "
                                                    value="{{ $exchangeRates[$cur] ?? '' }}" step="0.000001"
                                                    min="0" lang="en">
                                                <label>{{ $cur }} / {{ $exchangeBase }}</label>
                                                <div class="form-hint">1 {{ $exchangeBase }} =
                                                    {{ number_format($exchangeRates[$cur] ?? 0, 6, '.', '') }}
                                                    {{ $cur }}</div>
                                            </div>
                                        </div>
                                    @endforeach
                        ...
                        <div class="section-card-body">
                            ...
                            <form method="POST" action="{{ route('super.admin.settings.rate-limits.update') }}">
                                ...
                                @foreach ($groups as $groupKey => $groupLabel)
                                    <h6 class="mt-3 mb-2" style="font-size:13px;font-weight:600;color:var(--text)">
                                        {{ $groupLabel }}</h6>
                                    <div class="row g-3">
                                        @foreach ($groupMap[$groupKey] as $key)
                                            <div class="col-md-4">
                                                <div class="form-floating-group">
                                                    <input type="number" name="rate_limits[{{ $key }}]"
                                                        class="form-control" placeholder=" "
                                                        value="{{ $rateLimits[$key] ?? config('finance.rate_limits.' . $key, 120) }}"
                                                        min="1" max="10000">
                                                    <label><code>{{ $key }}</code></label>
                                                    <div class="form-hint">
                                                        {{ __('super-admin.rate_limit_per_minute') }}</div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                ...
                {{-- === CURRENCIES TAB === --}}
                <div x-show="tab === 'currencies'" x-cloak x-transition:enter.duration.200ms x-data="currenciesManager()"
                    x-init="init()">
                    <div class="section-card mb-4">
                        <div class="section-card-header">
                            <h5><i class="bi bi-currency-exchange"></i>{{ __('super-admin.currencies') }}</h5>
                            <div class="section-card-actions">
                                <x-button size="sm" icon="bi bi-arrow-counterclockwise" @click="resetDefault()" class="btn-ghost" style="padding:4px 10px;font-size:12px;border-radius:var(--radius-xs)">{{ __('super-admin.currencies_reset_default') }}</x-button>
                            </div>
                        </div>
                        <div class="section-card-body">
                            ...
                            <form method="POST" action="{{ route('super.admin.settings.currencies.update') }}"
                                id="currencies-form">
                                @csrf @method('PUT')
                                <input type="hidden" name="currencies" id="currencies-input"
                                    value="{{ json_encode($currenciesList) }}">
                                    ...
                                    <table class="data-table" id="currencies-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="col-actions" style="width:80px">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="col-actions">
                                                        <div class="cell-actions">
                                                            <x-button icon="bi bi-pencil" @click="editCurrency(index)" title="{{ __('general.edit') }}" style="width:28px;height:28px;padding:0;border-radius:var(--radius-xs);border:1px solid var(--border);background:transparent;color:var(--text-muted);font-size:12px" />
                                                            <x-button icon="bi bi-trash" @click="removeCurrency(index)" title="{{ __('general.delete') }}" style="width:28px;height:28px;padding:0;border-radius:var(--radius-xs);border:1px solid transparent;background:transparent;color:var(--danger);font-size:12px" />
                                                        </div>
                                                    </td>
                                                </tr>
                                            </template>
                                ...
                                <x-button variant="accent" icon="bi bi-plus-lg" class="mt-3" @click="addCurrency()">{{ __('super-admin.add_currency') }}</x-button>
                            ...
                            <div class="modal-content modal-custom">
                                <div class="modal-header">
                                    <h5 class="modal-title" style="font-size:16px;font-weight:600">
                                        ...
                                        <span
                                            x-text="editingIndex !== null ? '{{ __('super-admin.edit_currency') }}' : '{{ __('super-admin.add_currency') }}'"></span>
                                    </h5>
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        <label class="form-label-custom">{{ __('super-admin.currency_code') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="text" x-model="form.code" maxlength="3" class="form-custom"
                                            placeholder="USD" style="text-transform:uppercase" x-ref="codeInput">
                                        <div class="text-danger small mt-1" x-show="formError.code"
                                            x-text="formError.code"></div>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label-custom">{{ __('super-admin.currency_name') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="text" x-model="form.name" class="form-custom"
                                            placeholder="US Dollar">
                                        <div class="text-danger small mt-1" x-show="formError.name"
                                            x-text="formError.name"></div>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label-custom">{{ __('super-admin.currency_symbol') }}
                                            <span class="text-danger">*</span></label>
                                        <input type="text" x-model="form.symbol" class="form-custom"
                                            placeholder="$" style="max-width:100px">
                                        <div class="text-danger small mt-1" x-show="formError.symbol"
                                            x-text="formError.symbol"></div>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <x-button variant="outline" data-bs-dismiss="modal">{{ __('general.cancel') }}</x-button>
                                    <x-button variant="accent" icon="bi bi-check2" @click="confirmCurrency()">{{ __('general.confirm') }}</x-button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="tab === 'zakatPrices'" x-cloak x-transition:enter.duration.200ms>
                    @php
                        ...
                        $zakatGoldPerGram = (float) \App\Models\Setting::get(
                            'zakat.gold_per_gram',
                            config('zakat.prices.gold_per_gram', 0),
                        );
                        $zakatSilverPerGram = (float) \App\Models\Setting::get(
                            'zakat.silver_per_gram',
                            config('zakat.prices.silver_per_gram', 0),
                        );
                        $zakatDefaultKarat = (int) \App\Models\Setting::get(
                            'zakat.default_karat',
                            config('zakat.prices.default_karat', 24),
                        );
                    @endphp
                    <div class="section-card mb-4">
                        <div class="section-card-header">
                        ...
                        <div class="section-card-body">
                            ...
                            <form method="POST" action="{{ route('super.admin.settings.zakat-prices.update') }}">
                                @csrf @method('PUT')
                                <x-toggle-switch class="mb-3" name="zakat_manual_override" value="1"
                                    :checked="$zakatManualOverride" label="{{ __('super-admin.zakat_manual_override') }}"
                                    hint="{{ __('super-admin.zakat_manual_override_hint') }}" />
                                <div class="row g-3">
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <input type="number" name="zakat_gold_per_gram" class="form-control"
                                                placeholder=" " value="{{ $zakatGoldPerGram }}" step="0.01"
                                                min="0" lang="en">
                                            <label>{{ __('super-admin.zakat_gold_per_gram') }} (USD)</label>
                                            <div class="form-hint">{{ __('super-admin.zakat_gold_per_gram_hint') }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <input type="number" name="zakat_silver_per_gram" class="form-control"
                                                placeholder=" " value="{{ $zakatSilverPerGram }}" step="0.01"
                                                min="0" lang="en">
                                            <label>{{ __('super-admin.zakat_silver_per_gram') }} (USD)</label>
                                            <div class="form-hint">{{ __('super-admin.zakat_silver_per_gram_hint') }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-floating-group">
                                            <select name="zakat_gold_karat" class="form-control">
                                                @foreach ([24, 22, 21, 18, 14, 10] as $k)
                                                    <option value="{{ $k }}"
                                                        {{ $zakatDefaultKarat === $k ? 'selected' : '' }}>
                                                        {{ $k }} {{ __('zakat.gold_karat') }}</option>
                                                @endforeach
                                            </select>
                ...
                return {
                    ...
                    confirmCurrency() {
                        ...
                        if (this.editingIndex !== null) {
                            this.currencies[this.editingIndex] = {
                                code,
                                name,
                                symbol
                            };

<x-super-admin-layout>
    <x-slot:title>{{ __('super-admin.test_checklist') }} - {{ config('app.name') }}</x-slot>
    <x-slot:page-title>{{ __('super-admin.test_checklist') }}</x-slot>
    <x-slot:page-description>{{ __('super-admin.test_checklist_desc') }}</x-slot>
    ...
    <div class="test-checklist-app" x-data="checklistApp()" x-init="init()">
            ...
            <div class="header-stats">
                <div class="header-stat" @click="filterStatus = null">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-lbl">{{ __('super-admin.test_total') }}</span>
                    </div>
                </div>
                <div class="header-stat cursor-pointer" :class="{ active: filterStatus === 'passed' }" @click="toggleFilter('passed')">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-lbl">{{ __('super-admin.test_passed') }}</span>
                    </div>
                </div>
                <div class="header-stat cursor-pointer" :class="{ active: filterStatus === 'failed' }" @click="toggleFilter('failed')">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-lbl">{{ __('super-admin.test_failed') }}</span>
                    </div>
                </div>
                <div class="header-stat cursor-pointer" :class="{ active: filterStatus === 'skipped' }" @click="toggleFilter('skipped')">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-lbl">{{ __('super-admin.test_skipped') }}</span>
                    </div>
                </div>
                <div class="header-stat cursor-pointer" :class="{ active: filterStatus === 'pending' }" @click="toggleFilter('pending')">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-lbl">{{ __('super-admin.test_pending') }}</span>
                    </div>
                </div>
        ...
        <div class="checklist-toolbar">
            <div class="toolbar-search">
                ...
                <input type="text" x-model="searchQuery" @keydown.slash.prevent="$refs.searchInput.focus()"
                       ...
                       placeholder="{{ __('general.search') }}..." id="checklist-search">
                <button class="search-clear" x-show="searchQuery.length > 0" @click="searchQuery = ''; $refs.searchInput.focus()" type="button">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            <div class="toolbar-actions">
                <button class="tb-btn" @click="expandAll()" title="{{ __('super-admin.test_expand_all') }}">
                    <i class="bi bi-arrows-expand"></i>
                </button>
                <button class="tb-btn" @click="collapseAll()" title="{{ __('super-admin.test_collapse_all') }}">
                    <i class="bi bi-arrows-collapse"></i>
                </button>
                <button class="tb-btn tb-btn-danger" @click="confirmReset" title="{{ __('super-admin.test_reset') }}">
                    <i class="bi bi-arrow-counterclockwise"></i>
                </button>
                <form method="POST" action="{{ route('super.admin.test-checklist.export-markdown') }}" style="display:inline">
                    @csrf
                    <button class="tb-btn" type="submit" title="{{ __('super-admin.test_export_md') }}">
                        <i class="bi bi-filetype-md"></i>
                    </button>
                </form>
                <form method="POST" action="{{ route('super.admin.test-checklist.import-markdown') }}" style="display:inline"
                      onsubmit="return confirm('{{ __('super-admin.test_import_confirm') }}')">
                    @csrf
                    <button class="tb-btn" type="submit" title="{{ __('super-admin.test_import_md') }}">
        ...
        <form id="reset-form" method="POST" action="{{ route('super.admin.test-checklist.reset') }}" style="display:none">
            @csrf
        </form>
                                    ...
                                    <div>
                                        ...
                                        <div class="cat-meta">
                                            ...
                                            <span :class="catStats(categoryName).passed === catStats(categoryName).total ? 'text-success' : ''">
                                                <span x-text="catStats(categoryName).passed + '/' + catStats(categoryName).total"></span> {{ __('super-admin.test_passed') }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="check-item" :class="'item-' + item.status">
                                        ...
                                        <div class="item-body">
                                            ...
                                            <div class="item-actions-row">
                                                <button class="ia-btn" @click="toggleDetails(item.id)" x-show="item.details">
                                                    ...
                                                    <span x-text="showDetails === item.id ? '{{ __('super-admin.test_details') }}' : '{{ __('super-admin.test_details') }}'"></span>
                                                </button>
                                                <a href="#" class="ia-btn" @click.prevent="focusNotes(item.id)">
                                                    <i class="bi bi-pencil"></i>
                                                    <span x-text="item.notes ? '{{ __('general.edit') }}' : '{{ __('general.add') }}'"></span>
                                                </a>
                                            ...
                                            <div class="item-notes">
                                                <div class="notes-row">
                                                    <input type="text" x-model="item.notes" :id="'notes-' + item.id"
                                                           class="notes-field"
                                                           placeholder="{{ __('super-admin.test_notes_placeholder') }}"
                                        ...
                                        <div class="item-actions">
                                            <button class="st-btn st-pass" :class="{ active: item.status === 'passed' }"
                                                    @click="setStatus(item.id, 'passed')" title="{{ __('super-admin.test_mark_passed') }}">
                                                <i class="bi bi-check-lg"></i>
                                            </button>
                                            <button class="st-btn st-fail" :class="{ active: item.status === 'failed' }"
                                                    @click="setStatus(item.id, 'failed')" title="{{ __('super-admin.test_mark_failed') }}">
                                                <i class="bi bi-x-lg"></i>
                                            </button>
                                            <button class="st-btn st-skip" :class="{ active: item.status === 'skipped' }"
                                                    @click="setStatus(item.id, 'skipped')" title="{{ __('super-admin.test_mark_skipped') }}">
                                                <i class="bi bi-dash-lg"></i>
                                            </button>
                                            <button class="st-btn st-pend" :class="{ active: item.status === 'pending' }"
                                                    @click="setStatus(item.id, 'pending')" title="{{ __('super-admin.test_mark_pending') }}">
                                                <i class="bi bi-arrow-counterclockwise"></i>
                                            </button>
                                        </div>
                                    </div>
    ...
    <script>
        window.checklistApp = function () {
            return {
                ...
                tabStats: {!! json_encode($tabStats) !!},
                stats: {!! json_encode($stats) !!},
                itemsByCategory: {},

                init() {
                    @foreach ($groups as $category => $catItems)
                    this.itemsByCategory['{{ $category }}'] = {!! json_encode($catItems->map(fn($i) => [
                        'id' => $i->id,
                        'item_key' => $i->item_key,
                        'category' => $i->category,
                ...
                async setStatus(itemId, newStatus) {
                    ...
                    try {
                        const url = '{{ route("super.admin.test-checklist.update", 0) }}'.replace('0', itemId);
                ...
                async updateNotes(itemId, notes) {
                    try {
                        const url = '{{ route("super.admin.test-checklist.notes", 0) }}'.replace('0', itemId);
                        await fetch(url, {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                ...
                confirmReset() {
                    if (confirm('{{ __('super-admin.test_reset_confirm') }}')) {

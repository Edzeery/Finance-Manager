<x-super-admin-layout>
    ...
    <form action="{{ route('super.admin.workspace-roles.update', $role) }}" method="POST">
        @csrf @method('PUT')
                    ...
                    <div class="section-card-body">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="permission-grid">
                                    @foreach($groupPerms as $perm)
                                        <label class="permission-card {{ in_array($perm->id, $rolePermissions) ? 'has' : '' }} {{ $role->slug === 'workspace_admin' ? 'disabled' : '' }}">
                                            <input type="checkbox" name="permissions[]" value="{{ $perm->id }}"
                                                {{ in_array($perm->id, $rolePermissions) ? 'checked' : '' }}
                                                {{ $role->slug === 'workspace_admin' ? 'disabled' : '' }}
                                                class="perm-check"
                        ...
                        @if($role->slug === 'workspace_admin')
                            <div class="d-flex align-items-center gap-2" style="font-size:13px;background:var(--info-light);color:var(--info);border-radius:var(--radius-sm);padding:10px 14px">
                                ...
                                {{ __('super-admin.workspace_admin_permissions_locked') }}
                            </div>
                        @endif
                    </div>
                </div>

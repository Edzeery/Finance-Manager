<x-app-layout>
    <x-slot:title>{{ __('settings.activity_log') }} - {{ config('app.name') }}</x-slot>
    <x-slot:page-title>{{ __('settings.activity_log') }}</x-slot>
    <x-slot:page-description>{{ __('settings.activity_log_desc') ?? __('general.recent_activity') }}</x-slot>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <form method="GET" class="d-flex flex-wrap align-items-center gap-2">
            ...
            <x-clear-filters :filters="['action','search','date_from','date_to']" />
        </form>

        <div class="d-flex gap-2 align-items-center">
            <x-per-page :current="request('per_page', 15)" :preserve="['action','search','date_from','date_to']" />
        </div>
    </div>

    <div class="section-card">
        <div class="section-card-header">
            <h5><i class="bi bi-clock-history"></i>{{ __('settings.activity_log') }}</h5>
        </div>

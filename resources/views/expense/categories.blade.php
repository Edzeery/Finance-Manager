<x-app-layout>
        ...
        <div class="col-lg-4">
            <div class="card-custom">
                <div class="card-body p-4">
                    <h5 class="mb-3" style="font-size:15px; font-weight:600">{{ __('general.add') }} {{ __('expense.category') }}</h5>
                    <form action="{{ route('expense.categories.store') }}" method="POST">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-6">
                                ...
                                <input type="text" name="icon" class="form-custom" value="{{ old('icon', 'bi-cart') }}" maxlength="50" placeholder="bi-cart">
                            </div>
                            <div class="col-6">
                                <label class="form-label-custom">{{ __('general.color') }}</label>
                                <input type="color" name="color" class="form-custom" value="{{ old('color', '#EF4444') }}" style="height:42px; padding:4px">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label-custom">{{ __('expense.type') }}</label>
                            <select name="type" class="form-custom">
                                <option value="variable" {{ old('type') === 'variable' ? 'selected' : '' }}>{{ __('expense.variable') }}</option>
                                <option value="fixed" {{ old('type') === 'fixed' ? 'selected' : '' }}>{{ __('expense.fixed') }}</option>
                                <option value="periodic" {{ old('type') === 'periodic' ? 'selected' : '' }}>{{ __('expense.periodic') }}</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <tbody>
                                @foreach($categories as $cat)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge badge-custom badge-expense">{{ __("expense.{$cat->type}") }}</span>
                                        </td>
                                        <td>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="type" id="edit_type" class="form-custom">
                                <option value="variable">{{ __('expense.variable') }}</option>
                                <option value="fixed">{{ __('expense.fixed') }}</option>
                                <option value="periodic">{{ __('expense.periodic') }}</option>
                            </select>
                        </div>
                        <div>
    ...
    {{-- Hidden delete forms --}}
    @foreach($categories as $cat)
        <form id="delete-form-category-{{ $cat->id }}" action="{{ route('expense.categories.destroy', $cat) }}" method="POST" style="display:none">
            @csrf @method('DELETE')
        </form>
    @endforeach
    ...
    function editCategory(id) {
        ...
        document.getElementById('editForm').action = '{{ route('expense.categories.update', ':id') }}'.replace(':id', id);

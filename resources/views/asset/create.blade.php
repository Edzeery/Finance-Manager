                    ...
                    <form action="{{ route('asset.store') }}" method="POST">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" class="form-custom @error('name') is-invalid @enderror" required maxlength="255">
                            ...
                            <div class="col-md-6" id="field-quantity">
                                ...
                                <input type="number" step="0.0001" min="0" name="quantity" value="{{ old('quantity') }}" class="form-custom @error('quantity') is-invalid @enderror" placeholder="0">
                                @error('quantity') <div class="text-danger mt-1" style="font-size:13px">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-md-6" id="field-unit_price">
                                <label class="form-label-custom">{{ __('asset.unit_price') }}</label>
                                <div class="input-group">
                                    <input type="number" step="0.01" min="0" name="unit_price" value="{{ old('unit_price') }}" class="form-custom @error('unit_price') is-invalid @enderror" placeholder="0.00">
                                ...
                                @error('unit_price') <div class="text-danger mt-1" style="font-size:13px">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-md-6" id="field-total_value">
                                <label class="form-label-custom">{{ __('asset.total_value') }}</label>
                                <div class="input-group">
                                    <input type="number" step="0.01" min="0" name="total_value" id="total_value" value="{{ old('total_value') }}" class="form-custom @error('total_value') is-invalid @enderror" placeholder="0.00">
                                    <span class="input-group-text" style="background:var(--bg); border:1px solid var(--border); border-radius:0 8px 8px 0; color:var(--text-muted); font-size:13px">{{ config('finance.currency_symbol') }}</span>
                                </div>
                                @error('total_value') <div class="text-danger mt-1" style="font-size:13px">{{ $message }}</div> @enderror
                            </div>

                            <div class="col-md-6" id="field-bank_name" style="display:none">
                                <label class="form-label-custom">{{ __('asset.bank_name') }}</label>
                                <input type="text" name="bank_name" value="{{ old('bank_name') }}" class="form-custom" maxlength="255">
                            </div>

                            <div class="col-md-6" id="field-account_number" style="display:none">
                                <label class="form-label-custom">{{ __('asset.account_number') }}</label>
                                <input type="text" name="account_number" value="{{ old('account_number') }}" class="form-custom" maxlength="255">
                            </div>

                            <div class="col-6">
                                <div class="mb-1">
                                    <x-toggle-switch name="is_liquid" :checked="old('is_liquid', '1')" label="{{ __('asset.is_liquid') }}" hint="{{ __('asset.liquid_help') }}" />
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="mb-1">
                                    <x-toggle-switch name="is_zakatable" :checked="old('is_zakatable', '1')" label="{{ __('asset.is_zakatable') }}" hint="{{ __('asset.zakatable_help') }}" />
                                </div>
                            </div>

<x-app-layout>
    <x-slot:title>{{ __('income.edit') }} - {{ config('app.name') }}</x-slot>
    <x-slot:page-title>{{ __('income.edit') }}</x-slot>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="number" step="0.01" min="0" name="amount" value="{{ old('amount', $income->amount) }}" class="form-custom @error('amount') is-invalid @enderror" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="date" value="{{ old('date', $income->date->format('Y-m-d')) }}" class="form-custom @error('date') is-invalid @enderror" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="description" value="{{ old('description', $income->description) }}" class="form-custom" maxlength="1000">
                            </div>

                            <div class="col-12">
                                <div class="mb-1">
                                    <x-toggle-switch name="is_recurring" id="is_recurring" :checked="old('is_recurring', $income->is_recurring)" label="{{ __('income.is_recurring') }}" hint="{{ __('income.recurring_help') }}" />
                                </div>
                            </div>

                            <div id="recurringFields" class="col-12 row g-3" style="display:{{ old('is_recurring', $income->is_recurring) ? 'flex' : 'none' }}">
                                <div class="col-md-6">
                                    ...
                                    <select name="recurring_frequency" class="form-custom">
                                        @foreach(\App\Enums\RecurringFrequency::cases() as $freq)
                                            <option value="{{ $freq->value }}" {{ (old('recurring_frequency') ?? $income->recurring_frequency) === $freq->value ? 'selected' : '' }}>
                                                {{ __("income.{$freq->value}") }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="recurring_end_date" value="{{ old('recurring_end_date', $income->recurring_end_date?->format('Y-m-d')) }}" class="form-custom">
                                </div>
                            </div>

                            <div class="col-12">
                                <label class="form-label-custom">{{ __('income.notes') }}</label>
                                <textarea name="notes" class="form-custom" rows="3" maxlength="1000">{{ old('notes', $income->notes) }}</textarea>
                            </div>
                        </div>

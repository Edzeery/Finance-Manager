...
<head>
    ...
    <meta name="description" content="<?php echo e($metaDescription ?? config('app.name') . ' - ' . __('general.app_description')); ?>">
...
<body>
    <div class="app-layout" x-data="appLayout" :class="{ 'sidebar-collapsed': collapsed }">
        ...
        <div class="main-content" @click="if(window.innerWidth < 992 && event.target === event.currentTarget) closeSidebarMobile()">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                <?php
                    ...
                    $hasExpiredWithoutGrace = !$hasActive && auth()->user()->subscriptions()
                        ->withoutGlobalScopes()
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($hasExpiredWithoutGrace): ?>
                    <div x-data="{ show: true }" x-show="show" x-transition.duration.300
                        ...
                        <div>
                            ...
                            <a href="<?php echo e(route('account.subscriptions')); ?>" class="ms-2 fw-bold text-decoration-underline" style="color:#856404">
                                <?php echo e(__('subscription.renew') ?? 'تجديد'); ?>

                            </a>
                        </div>
                        <button @click="show = false" type="button" style="background:none;border:none;color:#856404;cursor:pointer;padding:2px 6px">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.command-palette','data' => ['items' => [
            ['title' => __('general.dashboard'), 'description' => __('general.go_to_dashboard'), 'icon' => 'bi bi-grid-1x2-fill', 'url' => route('dashboard')],
            ['title' => __('general.profile'), 'description' => __('general.manage_profile'), 'icon' => 'bi bi-person', 'url' => route('account.profile')],
            ['title' => __('general.settings'), 'description' => __('general.manage_settings'), 'icon' => 'bi bi-gear', 'url' => route('settings.index')],
            ['title' => __('transactions.add_income'), 'description' => __('transactions.record_new_income'), 'icon' => 'bi bi-plus-circle', 'url' => route('income.create')],
            ['title' => __('transactions.add_expense'), 'description' => __('transactions.record_new_expense'), 'icon' => 'bi bi-dash-circle', 'url' => route('expense.create')],
            ['title' => __('transactions.title'), 'description' => __('transactions.view_all'), 'icon' => 'bi bi-arrow-left-right', 'url' => route('transactions.index')],
            ['title' => __('budget.title'), 'description' => __('budget.manage'), 'icon' => 'bi bi-pie-chart', 'url' => route('budget.index')],
            ['title' => __('goal.title'), 'description' => __('goal.manage'), 'icon' => 'bi bi-flag', 'url' => route('goal.index')],
            ['title' => __('debt.title'), 'description' => __('debt.manage'), 'icon' => 'bi bi-credit-card-2-front', 'url' => route('debt.index')],
            ['title' => __('general.notifications'), 'description' => __('general.view_notifications'), 'icon' => 'bi bi-bell', 'url' => route('notifications.index')],
            ['title' => __('report.title'), 'description' => __('report.generate'), 'icon' => 'bi bi-file-earmark-bar-graph', 'url' => route('report.index')],
            ['title' => __('invoices.title'), 'description' => __('invoices.view_all'), 'icon' => 'bi bi-receipt', 'url' => route('account.invoices.index')],
